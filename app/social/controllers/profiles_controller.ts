...
export default class ProfilesController {
  ...
  async following({ auth, params, inertia, response }: HttpContext) {
    const profile = await Profile.query()
      .where('username', params.username)
      .select('id', 'username', 'avatar', 'displayName', 'followers_count', 'following_count')
      .preload('following', (query) => {
        query.preload('follower', (q) => {
          q.select('id', 'username', 'avatar', 'displayName')
        })

        query.preload('following', (q) => {
          q.select('id', 'username', 'avatar', 'displayName')

          if (auth.isAuthenticated) {
            q.withAggregate('followers', (q) => {
              q.where('follower_id', auth.user!.currentProfileId!).count('*').as('is_following')
            })
          }
        })
        query.orderBy('created_at', 'desc')
    ...
    return inertia.render('social/profile_following', { profile })
  }

  async followers({ auth, params, inertia, response }: HttpContext) {
    const profile = await Profile.query()
      .where('username', params.username)
      .select('id', 'username', 'avatar', 'displayName', 'followers_count', 'following_count')
    ...
    return inertia.render('social/profile_followers', { profile })
  }
}
